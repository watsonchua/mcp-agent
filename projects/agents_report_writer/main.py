...
app = MCPApp(name="report_writer_orchestrator")
...
async def example_usage():
    async with app.run() as orchestrator_app:
        ...
        # search_agent = Agent(
        #     name="searcher",
        #     instruction="""You are an agent with access to the internet. Your job is to formulate queries based on 
        #     the user's request, do a web search, and return the most relevant results and URLs.""",
        #     server_names=[],
        # )

        finder_agent = Agent(
            name="finder",
            instruction="""You are an agent with access to the filesystem, access to web search,
        ...
        style_enforcer = Agent(
            name="style_enforcer",
            instruction="""Analyze the report for adherence to style guidelines.
            Evaluate the narrative flow, clarity of expression, and tone. Suggest improvements to 
            enhance storytelling, readability, and engagement.""",
            server_names=["fetch"],
        )
        ...
        task = """Write a report to GovTech's Deputy Chief Executive on the background and Roadmap of Agentic AI. The report should be concise
        based on the following skeleton:

        1. What is Agentic AI?
            - Formal definition of Agentic AI
            - Layman understanding
        2. How is Agentic AI different from just calling LLMs?
            - Why is it useful?
            - What problems does it address?
            - What to other techniques like RAG/fine-tuning?
        3. What are the challenges?
            - Understanding of use case
            - Evals
            - Latency
        4. What has been done with this technology?
            - Industry use cases
            - Government use cases
        5. Where can we apply it?
            - Possible use cases for GovTech and Singapore government
        6 .How do we apply it?
            - Tools and frameworks: Langgraph, MCP-agents, smol-agents
            - Cloud solutions: AWS, Google, Azure
        7. What do we want to look into?
            - What can we do for capability development in this area?

        There are some existing links provided in references.md for your reference. Make use of them and use web search for more information if you need to.

        Write each section of the report (numbered item in the skeleton) to a different markdown file in the folder "agent_research" in this directory. Make reference to the existing content when writing.
        """
